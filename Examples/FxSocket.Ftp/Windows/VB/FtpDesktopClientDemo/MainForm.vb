...
'INSTANT VB NOTE: The variable size was renamed since Visual Basic does not handle local variables named the same as class members well:
		...
		Private Sub PopulateServerList(ByVal path As String)
			Dim items As FtpItemCollection = _ftp.GetList()
...
'INSTANT VB NOTE: The variable size was renamed since Visual Basic does not handle local variables named the same as class members well:
			...
			For c As Integer = 0 To items.Count - 1
				Dim row(4) As String
				Dim dirInfo As FtpItem = items(c)
		...
		Private Sub btConnect_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btConnect.Click
			...
			If dlg.ShowDialog() = System.Windows.Forms.DialogResult.OK Then
				...
				Try
					_ftp.BeginConnect(dlg.Server, dlg.Port, dlg.SslMode, AddressOf OnConnectFinish, dlg)
				Catch ex As Exception
					Log(ex)
					SetConnectedState(False)
				End Try
		...
		Private Sub OnConnectFinish(ByVal asyncResult As IAsyncResult)
			Try
				...
				If dlg.SslMode = SslMode.None Then
					lblServerType.Text = "FTP"
				Else
					lblServerType.Text = "FTP " & dlg.SslMode
				End If
			Catch ex As Exception
				Log(ex)
				SetConnectedState(False)
		...
		''' </summary>
		Private Sub OnStateChanged(ByVal sender As Object, ByVal e As FtpStateChangedEventArgs)
		...
		''' </summary>
		Private Sub OnScanning(ByVal sender As Object, ByVal e As FtpDirectoryScanningEventArgs)
		...
		''' </summary>
		Private Sub OnTransferProgress(ByVal sender As Object, ByVal e As FtpFileTransferProgressEventArgs)
			Dim info As String
		...
		''' </summary>
		Private Sub OnDeleteProgress(ByVal sender As Object, ByVal e As FtpDeleteProgressEventArgs)
		...
		''' </summary>
		Private Sub OnFileTransferIssue(ByVal sender As Object, ByVal e As FtpIssueEventArgs)
		...
		Private Sub cbModeZ_CheckedChanged(ByVal sender As Object, ByVal e As EventArgs) Handles cbModeZ.CheckedChanged
			...
			Else
				_ftp.TransferMode = FtpTransferMode.Stream
			End If
		End Sub
